class Api::V1::SubscriptionsController < Api::V1::BaseController
  ...
  def getHeartRate(update)
    @activities = @updateUser.sessions.where(:heartrate.empty? && :template false).order(completed_at: :desc)
  ...
  def getCalories(update)
    @activities = @updateUser.sessions.where(:calories.empty?  && :template false).order(completed_at: :desc)
    @activities.each do |activity|
      date = activity.created_at.strftime("%Y-%m-%d")
      start = activity.created_at.strftime("%H:%M:%S")
      finish = activity.completed_at.strftime("%H:%M:%S")
